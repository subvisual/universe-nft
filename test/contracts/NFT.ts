...
import type { NFT } from "@root/typechain-types";
...
describe("NFT", () => {
  ...
  beforeEach(async () => {
    ...
    nft = (await NFT.deploy("Subvisual NFT", "SNFT", "https://universe.subvisual.com/nft/{id}")) as NFT;
  });

  describe("setURI", () => {
    it("can be set by the operator", async () => {
      await nft.connect(owner).setURI("something else");
    });

    it("cannot be set by a non-operator", async () => {
      const action = nft.connect(alice).setURI("something else");
    ...
    it("can be set by an approved operator", async () => {
      ...
      await nft.connect(alice).setURI("something else");
    });
  });
